...
function Move-GeneratedFiles {
    if (-not(Test-Path "$docsFolder\create\commands")) {
        mkdir "$docsFolder\create\commands" -EA Continue | Out-Null
    }
...
function Generate-CommandReference($commandName, $order) {
    if (-not(Test-Path "$docsFolder\choco\commands")) {
        mkdir "$docsFolder\choco\commands" -EA Continue | Out-Null
    }
...
try {
    ...
    if (Test-Path($docsFolder)) {
        Remove-Item $docsFolder -Force -Recurse -EA SilentlyContinue
    }
    if (-not(Test-Path $docsFolder)) {
        mkdir $docsFolder -EA Continue | Out-Null
    }
    if (-not(Test-Path "$docsFolder\create\functions")) {
        mkdir "$docsFolder\create\functions" -EA Continue | Out-Null
    }
