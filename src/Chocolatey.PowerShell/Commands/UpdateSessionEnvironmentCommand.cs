...
namespace Chocolatey.PowerShell.Commands
{
    ...
    public sealed class UpdateSessionEnvironmentCommand : ChocolateyCmdlet
    {
        ...
        protected override void End()
        {
            ...
            if (calledByAlias)
            {
                WriteObject("Refreshing environment variables from the registry for powershell.exe. Please wait...");
            }
            else
            {
                WriteVerbose("Refreshing environment variables from the registry.");
            }
